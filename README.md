...
## UC-1 cold start on a fresh/demo machine (UC1-001) — one command

Brings up the **complete UC-1 stack** — local PostgreSQL → `jnpa_v3_local` →
migrations → seeded demo account → this gateway on `:8000` → the UC-1 dashboard
(`jnpa_poc_1`) on `:5173` — and verifies it against the real JNPA corpus.

```powershell
copy .env.uc1-local.example .env.uc1-local     # then set UC1_PG_PASSWORD
powershell -ExecutionPolicy Bypass -File scripts\uc1-up.ps1
```

Then open **http://localhost:5173**. The dashboard signs in automatically with
the seeded account (`admin`, password from `.env.uc1-local`).

| Flag | Effect |
|---|---|
| *(none)* | Start everything, verify, print a status table |
| `-StatusOnly` | Report and verify only. Starts nothing, writes nothing |
| `-SkipFrontend` | Postgres + database + gateway only (API work, CI) |
| `-Down` | Stop the gateway and dashboard **this script started**. Never drops data |

Exit codes: `0` up and verified · `1` a component failed to start · `2` the
stack is up but a verification did not pass.

**PowerShell, not `make`.** The Makefile targets below are POSIX-only and the
demo machines are Windows without Docker. `uc1-up.ps1` drives the host-native
path (local PostgreSQL + `uvicorn` + `vite`) that those machines can actually
run; it is additive and changes nothing about the Docker/`make` workflow.

### What it is idempotent about

Every step decides for itself whether there is work to do, so a second run is
safe and fast:

| Step | First run | Second run |
|---|---|---|
| PostgreSQL | `STARTED` (Windows service) | `ALREADY RUNNING` |
| `jnpa_v3_local` | `CREATED` | `EXISTS` |
| Corpus | `LOADED` from the dump | `ALREADY LOADED` — never reloaded |
| Migrations | `APPLIED` | ledger says applied; nothing runs |
| Demo account | `CREATED` | `EXISTS` — password left alone |
| Gateway / dashboard | `STARTED` | `ALREADY RUNNING` |

### Migrations — who applies what

The two migration directories are handled by their existing owners; the script
adds no third mechanism and re-runs nothing.

| Set | Applied by | When |
|---|---|---|
| `infra/postgres/v3` **0100–0127 + backfills** (incl. **0101**) | `scripts/migrate.py`, ledgered in `core.schema_migrations` | cold-start step 5 |
| **0036, 0037** (`infra/postgres/migrations`) | the same runner, via `--dir … --only 0036,0037` | cold-start step 5 |
| Marine **0038–0052** | the gateway itself — `gateway/marine_ext.py::ensure_marine_schema` and its sibling `ensure_*_schema` functions, gated on `JNPA_RUNTIME_DDL=1` | gateway boot |

`--only` also refuses an ambiguous version: `infra/postgres/migrations` contains
**both** `0038_marine_vessel_call.sql` and `0038_perf_pdf_upload.sql`, and the
ledger is keyed on version, so selecting `0038` is an error rather than a coin
toss.

### Where the data comes from

Neither repository ships a UC-1 seed or fixture, so the corpus is restored from
a `pg_dump` archive at `backups/jnpa_v3_local.dump` (gitignored — it holds real
JNPA operational data). On a networked developer machine, setting
`UC1_SOURCE_DSN` lets the script produce that archive itself. Two categories of
table are dumped **structure-only**, listed explicitly in
`UC1_DUMP_EXCLUDE_DATA`: UC-3 simulator telemetry (`truck_telemetry`,
`anpr_read`, `rfid_read` — 101 GB of the 103 GB source, regenerated by the
simulators, read by no UC-1 KPI) and driver personal data (`driver*`, `pdp`),
which a UC-1 marine demo has no use for.

### Readiness, not sleeps

Nothing waits a fixed number of seconds. PostgreSQL is polled with `pg_isready`;
the gateway with `GET /healthz` **checking the payload's `service` field**, so a
different process holding `:8000` cannot be mistaken for a healthy gateway; the
dashboard with an HTTP `GET` on its port. Each probe has a timeout and prints
the tail of the relevant log on expiry.

### Authentication is enforced

The cold start runs with `AUTH_ENABLED=true` and `AUTH_DEV_TOKENS=false`, and
the verification step logs in as the demo account to obtain the bearer it uses
for every other check — the same exchange the dashboard performs. Nothing is
bypassed.

> `gateway/users.py` enforces `MIN_PASSWORD_LENGTH = 8`, so the literal
> `admin/admin` of the ticket is rejected by `scripts/seed_auth_users.py`. The
> demo account keeps the username `admin` with an 8-character password from
> `.env.uc1-local`, rather than weakening that policy repo-wide.
