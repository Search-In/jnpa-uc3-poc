...
_DDL: list[str] = [
    ...
    # -------------------------------------------------------------------- RMS
    # RMS tables mirror the v3 RUNTIME shape the repository SQL targets
    # (report_id PK, machine_type / agent_pan / processing_end column names,
    # partial-unique igm_no arbiter, UNIQUE (report_id, sl_no)) — the previous
    # legacy-0031 shape here made every RMS insert fail on a fresh DB (audit).
    """CREATE TABLE IF NOT EXISTS core.rms_scan_report (
        report_id       bigserial PRIMARY KEY,
        id              bigint GENERATED BY DEFAULT AS IDENTITY,
        message_id      bigint REFERENCES core.customs_message(id) ON DELETE CASCADE,
        customs_house   text,
        shipping_line   text,
        agent_pan       text,
        igm_no          bigint,
    ...
    """CREATE TABLE IF NOT EXISTS core.rms_scan_container (
        id            bigserial,
        report_id     bigint NOT NULL REFERENCES core.rms_scan_report(report_id) ON DELETE CASCADE,
        igm_no        bigint,
        sl_no         integer,
        container_no  text NOT NULL,
        iso_valid     boolean NOT NULL DEFAULT true,
        ...
        goods_desc    text,
        created_at    timestamptz NOT NULL DEFAULT now(),
        CONSTRAINT uq_rms_container UNIQUE (report_id, sl_no))""",
    "CREATE INDEX IF NOT EXISTS idx_rms_cont_scan ON core.rms_scan_container (report_id)",
    "CREATE INDEX IF NOT EXISTS idx_rms_cont_no   ON core.rms_scan_container (container_no)",
    "CREATE INDEX IF NOT EXISTS idx_rms_cont_igm  ON core.rms_scan_container (igm_no)",
    # ---------------------------------------------------------- Shipping Bill
    ...
    # Joins mirror the canonical v3 view (0103): by be_no / smtp_no / report_id,
    # never by the ext id columns (second drift the audit found).
    "CREATE SCHEMA IF NOT EXISTS mart",
    """CREATE OR REPLACE VIEW mart.v_customs_container_status AS
        WITH cont AS (
            SELECT container_no, igm_no FROM core.igm_line_container
            UNION SELECT oc.container_no, o.igm_no FROM core.ooc_item oc
                  JOIN core.bill_of_entry_ooc o ON o.be_no = oc.be_no
            UNION SELECT sl.container_no, s.igm_no FROM core.smtp_container sl
                  JOIN core.smtp_permit s ON s.smtp_no = sl.smtp_no
            UNION SELECT rc.container_no, r.igm_no FROM core.rms_scan_container rc
