...
# UC3-030 disclosure strings. gateway/enforcement.py imports THIS module for
# the fine schedule, so importing enforcement back here would be circular; the
# strings are defined once here and re-exported by enforcement, which keeps a
# single source of truth without the cycle.
CHALLAN_AUTHORITY_NOTE = "issuance authority: JNPA/RTO — assumption A5"
CHALLAN_DISCLOSURE = (
        ...
        <section class="incident" style="page-break-after: {'always' if idx < total - 1 else 'auto'};">
          ...
          <div class="challan">
            ...
            <div class="sim-ribbon">
              SIMULATED — not a legally issued challan
              <span class="sim-note">{_esc(CHALLAN_AUTHORITY_NOTE)}</span>
            </div>
          ...
          <p class="sim-disclosure">{_esc(CHALLAN_DISCLOSURE)}</p>
        </section>
        """)
    body = "\n".join(pages) or f"""
...
<style>
  * {{ box-sizing: border-box; }}
  ...
  /* UC3-030: the SIMULATED ribbon must survive print, so it is a solid block with
     its own border rather than a background the printer may drop. */
  .sim-ribbon {{ margin:8px 0 0; padding:7px 10px; border:2px solid #D55E00;
                 border-radius:4px; background:#fff4e8; color:#8a3d00;
                 font-size:12px; font-weight:700; letter-spacing:.6px;
                 text-transform:uppercase; -webkit-print-color-adjust:exact;
                 print-color-adjust:exact; }}
  .sim-ribbon .sim-note {{ display:block; margin-top:2px; font-size:9.5px;
                           font-weight:600; letter-spacing:.2px;
                           text-transform:none; color:#8a3d00; }}
  .sim-disclosure {{ margin:6px 0 0; font-size:9px; line-height:1.4; color:#8a3d00;
                     border-top:1px dashed #e0b58a; padding-top:5px; }}
