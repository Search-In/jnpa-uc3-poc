...
export default function Home({ deviceId, plate }: { deviceId: string; plate?: string | null }) {
  ...
  const status: Status = (() => {
    if ((truck as any)?.elevated_scrutiny || unread > 0)
      return { tone: "alert", icon: "🔴", label: t("home.moving.alert", { defaultValue: "Alert" }) };
    if (/QUEUE|WAIT|GATE|IDLE|STOP/i.test(state) || (speed != null && speed <= 3))
      return { tone: "waiting", icon: "🟡", label: t("home.moving.waiting", { defaultValue: "Waiting" }) };
    return { tone: "moving", icon: "🟢", label: t("home.moving.moving", { defaultValue: "Moving" }) };
  })();
  ...
  const tile = (label: string, value: string, sub?: string) => (
    <div style={{ flex: "1 1 44%", background: "var(--surface2,#f1f4f9)", borderRadius: 14, padding: "12px 14px", minWidth: 0 }}>
      <div style={{ fontSize: 22, fontWeight: 700, lineHeight: 1.1 }}>{value}</div>
      <div className="muted" style={{ fontSize: 12, marginTop: 2 }}>{label}{sub ? ` · ${sub}` : ""}</div>
    </div>
  );
    ...
    <div style={{ padding: 12, paddingBottom: 24 }}>
      ...
      <div className="card" style={{ padding: 16, marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ width: 46, height: 46, borderRadius: "50%", background: "var(--blue,#2563eb)", color: "#fff",
            display: "grid", placeItems: "center", fontSize: 20, fontWeight: 700 }}>
            {(session.name || "D").trim().charAt(0).toUpperCase()}
          </span>
          <div style={{ minWidth: 0 }}>
            <div className="muted" style={{ fontSize: 12 }}>{t("home.welcome")}</div>
            <div style={{ fontSize: 20, fontWeight: 700, lineHeight: 1.1 }}>{session.name || t("home.driver", { defaultValue: "Driver" })}</div>
            <div style={{ fontSize: 14, fontWeight: 600, color: "var(--blue,#2563eb)", marginTop: 2 }}>
        ...
        {/* Big status pill */}
        <div style={{ marginTop: 14, background: tone.bg, borderRadius: 14, padding: "12px 16px",
          ...
          <div>
            ...
            <div className="muted" style={{ fontSize: 12 }}>
              ...
              {status.tone === "alert" && unread > 0 ? `${unread} ${t("home.newAlerts", { defaultValue: "new alerts" })}` : ""}
              {status.tone === "waiting" ? t("home.atGate", { defaultValue: "At gate / queue" }) : ""}
            </div>
          </div>
        </div>

        {session.status !== "ACTIVE" && (
          <button className="btn primary" style={{ marginTop: 12, width: "100%" }} onClick={() => navigate("/enrol")}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
        {tile(t("home.eta", { defaultValue: "ETA to gate" }), etaMin != null ? `${Math.round(etaMin)}m` : "—")}
        {tile(t("home.distance", { defaultValue: "Distance" }), remainingKm != null ? `${Number(remainingKm).toFixed(1)} km` : "—")}
        {tile(t("home.speed", { defaultValue: "Speed" }), speed != null ? `${Math.round(speed)}` : "—", "km/h")}
        {tile(t("home.parkingFree", { defaultValue: "Parking free" }), parkingFree != null ? String(parkingFree) : "—")}
      </div>

      {/* Live map preview */}
      <div className="card" style={{ padding: 0, overflow: "hidden", marginBottom: 12 }}>
        <div style={{ padding: "10px 14px 0", fontSize: 12, fontWeight: 600, color: "var(--muted)" }}>
          {t("home.mapPreview", { defaultValue: "LIVE POSITION" })}
        </div>
        <div style={{ height: 180 }}>
          <MiniMap corridor={corridor} gates={gates} truck={rec.lat != null ? { lat: rec.lat, lon: rec.lon } as any : null} />
        </div>
        <button className="btn ghost" style={{ width: "100%", borderRadius: 0 }} onClick={() => navigate("/map")}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button className="btn" style={{ flex: 1 }} onClick={() => navigate("/alerts")}>
          🔔 {t("home.alerts", { defaultValue: "Alerts" })}{unread > 0 ? ` (${unread})` : ""}
        </button>
      </div>
    </div>
  );
