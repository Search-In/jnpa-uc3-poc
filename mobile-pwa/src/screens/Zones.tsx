    ...
    try {
      ...
      for (const e of r.events || []) {
        if (e.event === "RESTRICTED_ENTRY")
          newWarns.push({
            text: t("zones.warnRestricted", { zone: e.zone_id }),
            kind: "restricted",
            at: Date.now(),
          });
        else if (e.event === "NO_PARKING_VIOLATION")
          newWarns.push({
            text: t("zones.warnNoParking", { zone: e.zone_id }),
            kind: "noparking",
            at: Date.now(),
          });
        else if (e.event === "ENTER")
          newWarns.push({
            text: t("zones.enter", { zone: e.zone_id }),
            kind: "enter",
            at: Date.now(),
          });
      }
  ...
  return (
    <div style={{ padding: 12 }}>
        ...
        {inside.length ? (
          inside.map((z) => (
            <div
              ...
              style={{
                fontWeight: 600,
                color: z.kind === "restricted" ? "var(--red, #c00)" : "var(--amber, #d80)",
              }}
            >
              {z.name} · {z.kind}
            </div>
      ...
      {warnings.length > 0 && (
        <div
          className="card"
          style={{ padding: 12, marginBottom: 12, borderLeft: "3px solid var(--red, #c00)" }}
        >
          ...
          {warnings.map((w, i) => (
            <div
              ...
              style={{
                fontSize: 13,
                color:
                  w.kind === "restricted" || w.kind === "noparking"
                    ? "var(--red, #c00)"
                    : undefined,
              }}
            >
      ...
      <div className="card" style={{ padding: 12, marginBottom: 8 }}>
        <div style={{ fontWeight: 600, color: "var(--red, #c00)", marginBottom: 4 }}>
          {t("zones.restricted")} ({restricted.length})
        </div>
        {restricted.map((z) => (
          <div key={z.id} style={{ fontSize: 13 }}>
            {z.name}
          </div>
        ))}
      </div>
      <div className="card" style={{ padding: 12 }}>
        <div style={{ fontWeight: 600, marginBottom: 4 }}>
          {t("zones.noParkingZones")} ({allowed.length})
        </div>
        {allowed.map((z) => (
          <div key={z.id} style={{ fontSize: 13 }}>
            {z.name}
