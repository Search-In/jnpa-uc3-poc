...
--   * Vehicles    — ACTIVE parking_transaction rows for the occupying trucks
...
WITH picks AS (
  ...
  JOIN core.parking_slot s ON s.facility_id = f.id
  WHERE s.availability_status = 'AVAILABLE'
)
UPDATE core.parking_slot s
...
WHERE s.id = p.id
  ...
  AND (SELECT count(*) FROM core.parking_slot x
...
INSERT INTO core.parking_transaction (vehicle_id, driver_id, facility_id, slot_id, entry_time, status)
SELECT s.vehicle_id,
       NULL,
       s.facility_id,
       s.id,
       ...
       'ACTIVE'
FROM core.parking_slot s
WHERE s.availability_status = 'OCCUPIED'
  AND s.vehicle_id LIKE 'MH04PK%'
  AND NOT EXISTS (
    SELECT 1 FROM core.parking_transaction t
    WHERE t.slot_id = s.id AND t.status = 'ACTIVE'
  );

-- 3) A few COMPLETED transactions for the History tab (entry + exit + duration).
INSERT INTO core.parking_transaction (vehicle_id, driver_id, facility_id, slot_id, entry_time, exit_time, duration, status)
...
INSERT INTO core.parking_event (event_type, vehicle_id, driver_id, facility_id, slot_id, detail, created_at)
...
WHERE NOT EXISTS (
  SELECT 1 FROM core.parking_event pe
...
-- Summary
SELECT 'occupied_slots' AS metric, count(*)::text AS value FROM core.parking_slot WHERE availability_status = 'OCCUPIED'
UNION ALL SELECT 'active_txns', count(*)::text FROM core.parking_transaction WHERE status = 'ACTIVE'
UNION ALL SELECT 'completed_txns', count(*)::text FROM core.parking_transaction WHERE status = 'COMPLETED'
UNION ALL SELECT 'violations', count(*)::text FROM core.parking_event
