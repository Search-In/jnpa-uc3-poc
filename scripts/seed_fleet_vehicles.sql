-- ===========================================================================
-- Vehicle Master (fleet registry) seed — core.vehicle (was jnpa.fleet_vehicles).
--
...
-- (core.vehicle enforces uniqueness via the partial unique index
--  uq_vehicle_vehicle_id ON (vehicle_id) WHERE vehicle_id IS NOT NULL, so the
--  conflict target carries the matching predicate. v3 renames the legacy
--  vehicle_number column to vehicle_no.)
...
ON CONFLICT (vehicle_id) WHERE vehicle_id IS NOT NULL DO NOTHING;
