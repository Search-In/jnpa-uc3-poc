...
-- v3: jnpa.transporters -> core.transporter (name -> company_name; uniqueness on
-- code is the partial unique index uq_transporter_code WHERE code IS NOT NULL).
INSERT INTO core.transporter (code, company_name, gstin, status)
...
JOIN core.transporter t ON t.code = v.code
...
WHERE t.code = 'TRP-1003'
  AND NOT EXISTS (SELECT 1 FROM core.transporter_blacklist b
                  WHERE b.transporter_id = t.id AND b.status = 'ACTIVE');
UPDATE core.transporter SET status = 'BLACKLISTED'
WHERE code = 'TRP-1003' AND status <> 'BLACKLISTED';

-- --- Accidents (one open, one resolved) -------------------------------------
INSERT INTO core.accident (accident_ref, accident_type, severity, lat, lon, location,
...
WHERE NOT EXISTS (SELECT 1 FROM core.accident x WHERE x.accident_ref = a.accident_ref);

INSERT INTO core.accident_event (accident_id, action, new_status, note, actor)
SELECT a.id, 'REPORTED', 'REPORTED', a.description, 'seed'
FROM core.accident a
WHERE a.accident_ref IN ('ACC-SEED01', 'ACC-SEED02')
  AND NOT EXISTS (SELECT 1 FROM core.accident_event e WHERE e.accident_id = a.id);

-- --- Camera-AI counts / trailer / container ---------------------------------
INSERT INTO core.camera_ai_count (camera_id, gate_id, vehicle_count, queue_count, class_counts, congestion_level, confidence)
SELECT * FROM (VALUES
...
WHERE NOT EXISTS (SELECT 1 FROM core.camera_ai_count x WHERE x.camera_id = c.camera_id);

INSERT INTO core.trailer_read (camera_id, gate_id, trailer_number, plate, vehicle_id, confidence)
SELECT 'CAM-G-NSICT-1', 'G-NSICT', 'HR55T7788', 'MH04AB1234', 'TRK-000123', 0.83
WHERE NOT EXISTS (SELECT 1 FROM core.trailer_read WHERE trailer_number = 'HR55T7788');

INSERT INTO core.container_read (camera_id, gate_id, container_number, iso_type, check_digit_ok, valid, plate, confidence)
SELECT 'CAM-G-NSICT-1', 'G-NSICT', 'CSQU3054383', '45G1', true, true, 'MH04AB1234', 0.90
WHERE NOT EXISTS (SELECT 1 FROM core.container_read WHERE container_number = 'CSQU3054383');

-- --- Document OCR sample ----------------------------------------------------
INSERT INTO core.document_ocr (doc_type, source_ref, fields, confidence, status, source)
SELECT 'LR', 'MH04AB1234',
       '{"lr_number":"LR-77321","consignor":"Bhagwati Roadlines","consignee":"JNPA Terminal","date":"2026-07-16"}'::jsonb,
       0.86, 'EXTRACTED', 'MOCK'
WHERE NOT EXISTS (SELECT 1 FROM core.document_ocr WHERE fields->>'lr_number' = 'LR-77321');

-- --- NVR device + channel map -----------------------------------------------
INSERT INTO core.nvr_device (id, name, vendor, host, port, protocol, channels, status, source)
VALUES ('NVR-G-NSICT', 'NSICT Gate NVR', 'Hikvision', '10.20.0.11', 554, 'RTSP', 8, 'ONLINE', 'CONFIG')
ON CONFLICT (id) DO NOTHING;

INSERT INTO core.nvr_camera_map (nvr_id, channel, camera_id, stream_url, codec, resolution, fps, status)
...
INSERT INTO core.trt_record (vehicle_id, plate, trip_id, gate_in_at, parking_at, loading_at, gate_out_at,
...
WHERE NOT EXISTS (SELECT 1 FROM core.trt_record WHERE trip_id = 'TRIP-SEED-1');

-- --- Bottleneck snapshot (top-3) --------------------------------------------
INSERT INTO core.bottleneck_snapshot (rank, segment_id, name, jam_factor, speed_kmh, free_flow_kmh, avg_delay_min, lat, lon)
SELECT * FROM (VALUES
...
WHERE NOT EXISTS (SELECT 1 FROM core.bottleneck_snapshot x WHERE x.segment_id = b.segment_id);

-- --- TT double-trip: one completed 2-leg cycle ------------------------------
INSERT INTO core.tt_trip (cycle_id, vehicle_id, driver_id, trip_seq, direction, origin, destination,
...
WHERE NOT EXISTS (SELECT 1 FROM core.tt_trip x WHERE x.cycle_id = 'TT-SEED-1' AND x.trip_seq = tt.trip_seq);
