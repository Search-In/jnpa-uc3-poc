"""Traffic persistence — raw-SQL repository over the shared async engine.

The ONLY layer that speaks SQL to ``core.traffic_reading``. Mirrors
:mod:`services.weather.repository`: reads on a plain ``connect()`` via the
``jnpa_shared.db`` helpers, writes through the committing helpers, no ORM.
Stateless apart from the DSN.

Every successful LIVE TomTom fetch is appended here (best-effort, by the
...
near a coordinate when both TomTom and the Redis cache are unavailable.

NOTE: distinct from ``core.traffic_snapshot`` (per-corridor-segment rows fed
by the simulation/ingest pipeline for the map overlay) — this table stores
point readings from the external TomTom provider. No overlap, no duplication.
"""
...
log = get_logger("services.traffic.repository")

# Coordinates are matched at 2 decimal places (~1.1 km) so nearby requests for
# the corridor share one reading history rather than fragmenting per-decimal.
...
_READING_COLS = ("id, latitude, longitude, current_speed, free_flow_speed, "
                 "congestion_level, delay_seconds, source, payload, created_at")


class TrafficRepository:
    """Raw-SQL persistence for core.traffic_reading. Stateless apart from the DSN."""
    ...
    async def insert_reading(
        ...
        *,
        ...
        row = await execute_returning(
            """INSERT INTO core.traffic_reading
                 (latitude, longitude, current_speed, free_flow_speed,
                  congestion_level, delay_seconds, source, payload)
               VALUES (:lat, :lon, :current_speed, :free_flow_speed,
                       :congestion_level, :delay_seconds, :source,
    ...
    async def count_readings(
        ...
        *,
        ...
        where = f"WHERE {_COORD_MATCH_SQL}" if latitude is not None and longitude is not None else ""
        row = await fetch_one(
            f"SELECT count(*) AS n FROM core.traffic_reading {where}",
            {"lat": latitude, "lon": longitude},
            dsn=self._dsn,
        )
