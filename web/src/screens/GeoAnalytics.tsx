...
export default function GeoAnalytics({ defaultTab = "zones" }: { defaultTab?: TabKey }) {
  ...
  const insideQ = useQuery({ queryKey: ["geo-inside"], queryFn: () => api.geoVehiclesInZones(), refetchInterval: 8000 });
  ...
  const eventsQ = useQuery({ queryKey: ["geo-events"], queryFn: () => api.geoEvents(undefined, 200) });

  const updatedAt = Math.max(zonesQ.dataUpdatedAt || 0, insideQ.dataUpdatedAt || 0, violQ.dataUpdatedAt || 0);
    ...
    <PageContainer>
      ...
      <div className="px-4 pt-3">
        <StatGrid className="lg:grid-cols-5">
          <StatCard icon={MapPinned} label="Active Zones" value={zonesQ.data?.zones?.length ?? "—"} tone="info" loading={zonesQ.isLoading} />
      ...
      <div className="px-4 py-3">
        <SegmentedTabs
          ...
          tabs={[
            ...
            { key: "inside", label: "Vehicles in Zone", icon: MapPinned, count: insideQ.data?.vehicles?.length },
        ...
        {tab === "inside" && (
          <Card className="overflow-hidden">
            <InsideTable rows={insideQ.data?.vehicles ?? []} status={insideQ} onRetry={() => insideQ.refetch()} />
          </Card>
        )}
        {tab === "events" && <EventsTimeline status={eventsQ} rows={eventsQ.data?.events ?? []} onRetry={() => eventsQ.refetch()} />}
        {tab === "violations" && (
          <Card className="overflow-hidden">
            <ViolationsTable rows={violQ.data?.violations ?? []} status={violQ} onRetry={() => violQ.refetch()} />
          </Card>
        )}
        {tab === "ai" && (
          <Card className="overflow-hidden">
...
function InsideTable({ rows, status, onRetry }: { rows: GeoVehicleInZone[]; status: any; onRetry: () => void }) {
  const columns: Column<GeoVehicleInZone>[] = [
    ...
    { key: "entry", header: "Entered", className: "text-muted-foreground", render: (v) => (v.entry_time ? fmtDateTimeIST(v.entry_time) : "—") },
    { key: "dwell", header: "Dwell", align: "right", className: "tabular-nums", render: (v) => `${Math.round(v.dwell_s / 60)}m` },
    { key: "state", header: "State", render: (v) => <StatusChip label={v.violated ? "VIOLATION" : "OK"} tone={v.violated ? "critical" : "ok"} /> },
  ];
  return (
    <DataTable
      columns={columns}
...
function EventsTimeline({ rows, status, onRetry }: { rows: GeofenceEvent[]; status: any; onRetry: () => void }) {
  const [q, setQ] = useState("");
  const [limit, setLimit] = useState(20);
  const filtered = useMemo(() => {
    const list = rows.filter((e) => e.event_type === "ENTER" || e.event_type === "EXIT");
    const query = q.trim().toLowerCase();
    return (query ? list.filter((e) => `${e.vehicle_id ?? ""} ${e.zone_id ?? ""}`.toLowerCase().includes(query)) : list).sort(
      (a, b) => Date.parse(b.created_at) - Date.parse(a.created_at),
    );
  }, [rows, q]);

  if (status.isLoading) return <Card className="p-0"><LoadingState /></Card>;
  if (status.isError) return <Card className="p-0"><ErrorState onRetry={onRetry} /></Card>;
        ...
        <>
            ...
            {filtered.slice(0, limit).map((e) => {
              ...
              return (
                <li key={e.id} className="relative flex gap-3 pb-4 last:pb-0">
                  ...
                  >
                    {enter ? <LogIn className="h-2.5 w-2.5 text-white" /> : <LogOut className="h-2.5 w-2.5 text-white" />}
                  </span>
                  <div className="ml-4 flex flex-1 flex-wrap items-center gap-x-2 gap-y-0.5">
                    <StatusChip label={e.event_type} tone={enter ? "warn" : "ok"} />
                    <span className="font-mono text-[13px] font-medium text-foreground">{e.vehicle_id}</span>
                    <span className="text-[13px] text-muted-foreground">→ {e.zone_id}</span>
                    {e.dwell_seconds != null && <span className="text-[11px] text-muted-foreground">· dwell {Math.round(e.dwell_seconds / 60)}m</span>}
                    <span className="ml-auto text-[11px] text-muted-foreground" title={fmtDateTimeIST(e.created_at)}>{relativeAge(e.created_at)}</span>
                  </div>
                </li>
              );
            })}
          ...
          {limit < filtered.length && (
            <div className="border-t border-border px-4 py-2 text-center">
              <button onClick={() => setLimit((l) => l + 20)} className="text-[12px] font-semibold text-primary hover:underline">
...
function ViolationsTable({ rows, status, onRetry }: { rows: GeofenceEvent[]; status: any; onRetry: () => void }) {
  const columns: Column<GeofenceEvent>[] = [
    { key: "type", header: "Violation", render: (e) => <StatusChip label={e.violation_type ?? "—"} tone="critical" /> },
    { key: "vehicle", header: "Vehicle", className: "font-mono", render: (e) => e.vehicle_id ?? "—" },
    { key: "driver", header: "Driver", render: (e) => e.driver_id ?? "—" },
    { key: "zone", header: "Zone", render: (e) => e.zone_id ?? "—" },
    { key: "dwell", header: "Dwell", align: "right", className: "tabular-nums", render: (e) => (e.dwell_seconds != null ? `${Math.round(e.dwell_seconds / 60)}m` : "—") },
    { key: "when", header: "When", className: "text-muted-foreground", render: (e) => fmtDateTimeIST(e.created_at) },
  ];
  return (
    <DataTable
      columns={columns}
      ...
      search={(e, q) => `${e.violation_type ?? ""} ${e.vehicle_id ?? ""} ${e.zone_id ?? ""}`.toLowerCase().includes(q)}
...
function AiTable({ rows, status, onRetry }: { rows: AiEvent[]; status: any; onRetry: () => void }) {
  const columns: Column<AiEvent>[] = [
    { key: "type", header: "AI Event", render: (e) => <StatusChip label={e.event_type} tone="warn" /> },
    { key: "vehicle", header: "Vehicle", className: "font-mono", render: (e) => e.vehicle_id ?? "—" },
    { key: "driver", header: "Driver", render: (e) => e.driver_id ?? "—" },
    { key: "location", header: "Location", className: "text-muted-foreground", render: (e) => summariseLocation(e.location) },
    { key: "when", header: "When", className: "text-muted-foreground", render: (e) => fmtDateTimeIST(e.created_at) },
  ];
  return (
    <DataTable
...
function HeatmapTab({ violations }: { violations: GeofenceEvent[] }) {
  ...
  const snapsQ = useQuery({ queryKey: ["snapshots"], queryFn: () => getAdapter().trafficSnapshots(), refetchInterval: 8000 });
  const zonesQ = useQuery({ queryKey: ["zones"], queryFn: () => getAdapter().zones() });
  const trucksQ = useQuery({ queryKey: ["trucks", "live-map"], queryFn: () => getAdapter().trucks(undefined, 500), refetchInterval: 5000 });
  ...
  const byZone = useMemo(() => {
    ...
    return Array.from(m.entries()).map(([name, count]) => ({ name: name.slice(0, 14), count })).sort((a, b) => b.count - a.count);
  }, [violations]);
  ...
  return (
    <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
      ...
      <Card className="p-3">
        ...
        {byZone.length === 0 ? (
          <div className="flex h-40 items-center justify-center text-sm text-muted-foreground">No violations to plot.</div>
        ) : (
          <div className="h-[460px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={byZone} layout="vertical" margin={{ top: 4, right: 12, left: 4, bottom: 4 }}>
