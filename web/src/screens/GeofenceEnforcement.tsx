...
function ZonesTab() {
  ...
  return (
    <>
      ...
      <AsyncBoundary status={q} isEmpty={!rows.length} onRetry={() => q.refetch()}
        empty={<EmptyState>No active zones.</EmptyState>}>
        <div className="mb-2 text-[11px] text-muted-foreground">
          Detection source: <span className="font-mono">{q.data?.source}</span> · {rows.length} enforced
        </div>
        <div className="overflow-x-auto rounded-md border border-border">
          <table className="w-full text-xs">
            <thead className="bg-muted/50 text-left text-muted-foreground">
              <tr><th className="px-3 py-2">Zone</th><th className="px-3 py-2">ID</th><th className="px-3 py-2">Kind</th><th className="px-3 py-2 text-right">Vertices</th></tr>
            </thead>
            <tbody>
              {rows.map((z) => (
                <tr key={z.id} className="border-t border-border/60">
                  ...
                  <td className="px-3 py-1.5"><Badge className={cn("text-[10px]", z.kind === "restricted" ? CRIT : WARN)}>{z.kind}</Badge></td>
...
function InsideTab() {
  const q = useQuery({ queryKey: ["geo-inside"], queryFn: () => api.geoVehiclesInZones(), refetchInterval: 8000 });
    ...
    <>
      ...
      <AsyncBoundary status={q} isEmpty={!rows.length} onRetry={() => q.refetch()}
        ...
        <div className="overflow-x-auto rounded-md border border-border">
          <table className="w-full text-xs">
            <thead className="bg-muted/50 text-left text-muted-foreground">
              <tr><th className="px-3 py-2">Vehicle</th><th className="px-3 py-2">Zone</th><th className="px-3 py-2 text-right">Dwell</th><th className="px-3 py-2">State</th></tr>
            </thead>
            <tbody>
              {rows.map((v, i) => (
                <tr key={`${v.vehicle_id}-${v.zone_id}-${i}`} className="border-t border-border/60">
                  ...
                  <td className="px-3 py-1.5 text-right tabular-nums">{Math.round(v.dwell_s / 60)}m</td>
                  <td className="px-3 py-1.5"><Badge className={cn("text-[10px]", v.violated ? CRIT : OK)}>{v.violated ? "VIOLATION" : "OK"}</Badge></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function EventsTab() {
  ...
  const rows = (q.data?.events ?? []).filter((e) => e.event_type === "ENTER" || e.event_type === "EXIT");
  return (
    <>
      <div className="mb-2 flex justify-end">
        <LastUpdated at={q.dataUpdatedAt} isFetching={q.isFetching && !q.isLoading} />
      ...
      <AsyncBoundary status={q} isEmpty={!rows.length} onRetry={() => q.refetch()}
        ...
        <div className="overflow-x-auto rounded-md border border-border">
          <table className="w-full text-xs">
            <thead className="bg-muted/50 text-left text-muted-foreground">
              <tr><th className="px-3 py-2">Type</th><th className="px-3 py-2">Vehicle</th><th className="px-3 py-2">Zone</th><th className="px-3 py-2 text-right">Dwell</th><th className="px-3 py-2">When</th></tr>
            </thead>
            <tbody>
              {rows.map((e) => (
                <tr key={e.id} className="border-t border-border/60">
                  <td className="px-3 py-1.5"><Badge className={cn("text-[10px]", evBadge(e.event_type))}>{e.event_type}</Badge></td>
                  <td className="px-3 py-1.5 font-mono">{e.vehicle_id}</td>
                  <td className="px-3 py-1.5">{e.zone_id}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{e.dwell_seconds != null ? `${Math.round(e.dwell_seconds / 60)}m` : "—"}</td>
                  <td className="px-3 py-1.5 text-muted-foreground">{fmtDateTimeIST(e.created_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <>
      ...
      <AsyncBoundary status={q} isEmpty={!rows.length} onRetry={() => q.refetch()}
        ...
        <div className="overflow-x-auto rounded-md border border-border">
          <table className="w-full text-xs">
            <thead className="bg-muted/50 text-left text-muted-foreground">
              <tr><th className="px-3 py-2">Violation</th><th className="px-3 py-2">Vehicle</th><th className="px-3 py-2">Driver</th><th className="px-3 py-2">Zone</th><th className="px-3 py-2 text-right">Dwell</th><th className="px-3 py-2">When</th></tr>
            </thead>
            <tbody>
              {rows.map((e) => (
                <tr key={e.id} className="border-t border-border/60">
                  <td className="px-3 py-1.5"><Badge className={cn("text-[10px]", CRIT)}>{e.violation_type}</Badge></td>
                  <td className="px-3 py-1.5 font-mono">{e.vehicle_id}</td>
    ...
    <>
      ...
      <AsyncBoundary status={q} isEmpty={!rows.length} onRetry={() => q.refetch()}
        ...
        <div className="overflow-x-auto rounded-md border border-border">
          <table className="w-full text-xs">
            <thead className="bg-muted/50 text-left text-muted-foreground">
              <tr><th className="px-3 py-2">AI Event</th><th className="px-3 py-2">Vehicle</th><th className="px-3 py-2">Location</th><th className="px-3 py-2">When</th></tr>
            </thead>
            <tbody>
              {rows.map((e) => (
                <tr key={e.id} className="border-t border-border/60">
                  <td className="px-3 py-1.5"><Badge className={cn("text-[10px]", WARN)}>{e.event_type}</Badge></td>
                  <td className="px-3 py-1.5 font-mono">{e.vehicle_id ?? "—"}</td>
                  <td className="px-3 py-1.5 text-muted-foreground">{JSON.stringify(e.location)}</td>
                  <td className="px-3 py-1.5 text-muted-foreground">{fmtDateTimeIST(e.created_at)}</td>
                </tr>
              ))}
            </tbody>
    ...
    <div className="h-full overflow-y-auto p-4">
      ...
      <Card>
        <CardHeader className="flex-row items-center gap-2">
          {tab === "zones" ? <MapPinned className="h-4 w-4 text-muted-foreground" /> :
           tab === "inside" ? <MapPinned className="h-4 w-4 text-muted-foreground" /> :
           tab === "events" ? <LogIn className="h-4 w-4 text-muted-foreground" /> :
           tab === "violations" ? <TriangleAlert className="h-4 w-4 text-muted-foreground" /> :
