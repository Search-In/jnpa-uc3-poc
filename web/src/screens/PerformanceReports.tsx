...
function Toolbar({ onExport }: { onExport: () => void }) {
  return (
    <div className="ml-auto flex items-center gap-2">
      <button type="button" onClick={onExport} className={BTN}>Export CSV</button>
...
export default function PerformanceReports() {
  ...
  const metaQ = useQuery({ queryKey: ["perf-meta"], queryFn: () => api.perfMeta(), staleTime: 60_000 });
      ...
      <div className="flex flex-col gap-4 p-4">
        <SegmentedTabs
          tabs={[
            ...
            ...(IS_ADMIN ? [{ key: "upload" as TabKey, label: "Data Upload", icon: UploadCloud }] : []),
          ]}
          value={tab}
          onChange={setTab}
        />
...
function OverviewTab() {
  ...
  const statsQ = useQuery({ queryKey: ["perf-stats"], queryFn: () => api.perfStats(), refetchInterval: POLL_MS });
  ...
  return (
    <div className="flex flex-col gap-4">
      <StatGrid className="lg:grid-cols-4 xl:grid-cols-7">
        <StatCard icon={Container} label="Container TEUs (day)" value={num(m.total_teus)} tone="info"
          loading={kpiLoading} sub={sub(deltaSub(d.total_teus))} />
        <StatCard icon={Layers} label="Throughput (tonnes)" value={numInt(m.total_tonnes)} tone="info"
          loading={kpiLoading} sub={sub(deltaSub(d.total_tonnes))} />
        <StatCard icon={Ship} label="Vessel calls" value={num(m.vessel_calls)} tone="neutral"
          loading={kpiLoading} sub={sub(deltaSub(d.vessel_calls))} />
        <StatCard icon={GaugeCircle} label="Yard occupancy" value={pctStr(m.yard_occupancy_pct)}
          tone={occTone(m.yard_occupancy_pct)} loading={kpiLoading} sub={sub(deltaSub(d.yard_occupancy_pct, "%"))} />
        <StatCard icon={TimerReset} label="Import pendency" value={num(m.total_pendency_teus)} tone="warn"
          loading={kpiLoading} sub="ICD + CFS (TEUs)" />
        <StatCard icon={Truck} label="Gate total" value={num(m.gate_total_teus)} tone="ok"
          loading={kpiLoading} sub={sub(deltaSub(d.gate_total_teus))} />
        <StatCard icon={Snowflake} label="Reefer available" value={num(m.reefer_available_slots)} tone="ok"
          loading={kpiLoading} sub={m.reefer_total_slots ? `of ${num(m.reefer_total_slots)} slots` : undefined} />
      </StatGrid>

      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card className="p-3">
          <h2 className="mb-2 text-sm font-semibold text-foreground">JN Port container TEUs — daily trend</h2>
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={series} margin={{ top: 6, right: 10, bottom: 0, left: -12 }}>
                ...
                <XAxis dataKey="day" tick={CHART_AXIS} interval="preserveStartEnd" minTickGap={28} />
                <YAxis tick={CHART_AXIS} width={44} />
                <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} />
                <Area type="monotone" dataKey="teus" name="TEUs" stroke={STATUS.info}
                  fill="url(#teuGrad)" strokeWidth={2} isAnimationActive={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
        <Card className="p-3">
          <h2 className="mb-2 text-sm font-semibold text-foreground">Gate movements — daily IN / OUT (TEUs)</h2>
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={series} margin={{ top: 6, right: 10, bottom: 0, left: -12 }}>
                ...
                <Bar dataKey="gate_in" name="Gate IN" fill={STATUS.ok} radius={[2, 2, 0, 0]} isAnimationActive={false} />
                <Bar dataKey="gate_out" name="Gate OUT" fill={STATUS.warning} radius={[2, 2, 0, 0]} isAnimationActive={false} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
...
function sub(d?: { text: string; tone: Tone }) {
  ...
  const colour = d.tone === "ok" ? STATUS.ok : d.tone === "critical" ? STATUS.critical : STATUS.unknown;
...
function DailyTab({ dates, latest }: { dates: string[]; latest: string | null }) {
  ...
  const statusCols: Column<any>[] = [
    { key: "terminal_code", header: "Terminal", render: (r) => <span className="font-semibold">{r.terminal_code}</span> },
    { key: "yard_occupancy_pct", header: "Yard occ.", align: "right",
      render: (r) => <StatusChip label={pctStr(r.yard_occupancy_pct)} tone={occTone(r.yard_occupancy_pct)} /> },
    { key: "yard_total_teus", header: "Yard TEUs", align: "right", render: (r) => num(r.yard_total_teus) },
    { key: "icd_pendency_teus", header: "ICD pend.", align: "right", render: (r) => num(r.icd_pendency_teus) },
    { key: "cfs_pendency_teus", header: "CFS pend.", align: "right", render: (r) => num(r.cfs_pendency_teus) },
    { key: "gate_total_teus", header: "Gate total", align: "right", render: (r) => num(r.gate_total_teus) },
    { key: "reefer_available_slots", header: "Reefer avail.", align: "right", render: (r) => num(r.reefer_available_slots) },
  ];
  const vesselCols: Column<any>[] = [
    { key: "terminal_code", header: "Terminal", render: (r) => <span className="font-semibold">{r.terminal_code}</span> },
    { key: "berth_no", header: "Berth" },
    { key: "via_no", header: "Voyage", render: (r) => <span className="font-mono">{r.via_no || "—"}</span> },
    { key: "vessel_name", header: "Vessel", render: (r) => <span className="font-mono">{r.vessel_name}</span> },
    { key: "cargo_commodity", header: "Cargo" },
    { key: "berthed_on", header: "Berthed", render: (r) => (r.berthed_on ? String(r.berthed_on).replace("T", " ").slice(0, 16) : "—") },
  ];
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card className="p-3">
        <h2 className="mb-2 text-sm font-semibold text-foreground">Container terminal TEUs (day) — by terminal</h2>
        <div className="h-[260px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={traffic.filter((r) => r.terminal_code !== "JN_PORT" && r.imp_teus != null)}
              ...
              <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} />
              <Legend wrapperStyle={{ fontSize: 11 }} />
              <Bar dataKey="imp_teus" name="Import" stackId="a" fill={STATUS.info} isAnimationActive={false} />
              <Bar dataKey="exp_teus" name="Export" stackId="a" fill={STATUS.ok} isAnimationActive={false} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </Card>
            ...
            <Toolbar
              onExport={() =>
                exportCsv(
                  ...
                  ["terminal", "yard_occupancy_pct", "yard_total_teus", "icd_pendency", "cfs_pendency", "gate_total", "reefer_available"],
                  status.map((r) => [r.terminal_code, r.yard_occupancy_pct, r.yard_total_teus, r.icd_pendency_teus, r.cfs_pendency_teus, r.gate_total_teus, r.reefer_available_slots]),
                )
              }
            />
          }
      ...
      <Card className="p-0">
        ...
        <DataTable
          ...
          search={(r, q) => `${r.vessel_name} ${r.terminal_code} ${r.cargo_commodity} ${r.via_no}`.toLowerCase().includes(q)}
...
function MonthlyTab() {
  ...
  const cols: Column<any>[] = [
    ...
    { key: "vessel_calls", header: "Vessel calls", align: "right", render: (r) => num(r.vessel_calls) },
    { key: "discharge_teus", header: "Discharge", align: "right", render: (r) => num(r.discharge_teus) },
    { key: "load_teus", header: "Load", align: "right", render: (r) => num(r.load_teus) },
    { key: "total_teus", header: "Total TEUs", align: "right", render: (r) => <span className="font-semibold">{num(r.total_teus)}</span> },
  ];
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2">
        <label className="text-[12px] font-medium text-muted-foreground">Terminal</label>
        <FilterSelect label="Terminal" value={terminal} onChange={setTerminal}
        ...
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chart} margin={{ top: 6, right: 10, bottom: 0, left: 0 }}>
              ...
              <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} />
              <Legend wrapperStyle={{ fontSize: 11 }} />
              <Bar dataKey="disch" name="Discharge" stackId="a" fill={STATUS.info} isAnimationActive={false} />
              <Bar dataKey="load" name="Load" stackId="a" fill={STATUS.ok} isAnimationActive={false} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </Card>
      <Card className="p-0">
        <DataTable
          ...
          pageSize={13}
          toolbar={
            <Toolbar onExport={() => exportCsv(`monthly-teu-${terminal}.csv`,
              ["month", "fiscal_year", "vessel_calls", "discharge_teus", "load_teus", "total_teus"],
              rows.map((r) => [`${r.month_label} ${r.year_label}`, r.fiscal_year, r.vessel_calls, r.discharge_teus, r.load_teus, r.total_teus]))} />
          }
        />
      </Card>
    </div>
...
function LdbTab({ months }: { months: string[] }) {
  ...
  const facCols: Column<any>[] = [
    { key: "facility_type", header: "Type", render: (r) => <StatusChip label={r.facility_type} tone={r.facility_type === "CFS" ? "info" : "neutral"} /> },
    { key: "facility_name", header: "Facility" },
    { key: "dwell_hours", header: "Dwell (Mar)", align: "right", render: (r) => hrs(r.dwell_hours) },
    { key: "dwell_hours_prev", header: "Dwell (Feb)", align: "right", render: (r) => hrs(r.dwell_hours_prev) },
  ];
  const congCols: Column<any>[] = [
    ...
    { key: "pct_containers", header: "% containers", align: "right", render: (r) => pctStr(r.pct_containers) },
    { key: "congestion_level", header: "Congestion", render: (r) => <StatusChip label={r.congestion_level} tone={congTone(r.congestion_level)} /> },
  ];

  return (
    <div className="flex flex-col gap-4">
      <div className="text-[12px] text-muted-foreground">
        NLDS / LDB Analytics — report month <span className="font-semibold">{month || "—"}</span> (vs previous month)
      </div>
      <Card className="p-3">
        <h2 className="mb-2 text-sm font-semibold text-foreground">Port dwell time (overall) — import vs export, by terminal</h2>
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={dwellChart} margin={{ top: 6, right: 10, bottom: 0, left: -8 }}>
              ...
              <Legend wrapperStyle={{ fontSize: 11 }} />
              <Bar dataKey="import" name="Import" fill={STATUS.info} radius={[2, 2, 0, 0]} isAnimationActive={false} />
              <Bar dataKey="export" name="Export" fill={STATUS.warning} radius={[2, 2, 0, 0]} isAnimationActive={false} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card className="p-0">
          <div className="border-b border-border px-3 py-2 text-sm font-semibold text-foreground">CFS / ICD facility dwell</div>
          <DataTable
            columns={facCols}
            rows={facRows}
            ...
            pageSize={10}
            toolbar={
              <Toolbar onExport={() => exportCsv(`ldb-facility-dwell-${month}.csv`,
                ["type", "facility", "dwell_hours", "dwell_hours_prev"],
                facRows.map((r) => [r.facility_type, r.facility_name, r.dwell_hours, r.dwell_hours_prev]))} />
            }
          />
        </Card>
        <Card className="p-0">
          <div className="border-b border-border px-3 py-2 text-sm font-semibold text-foreground">Import congestion clusters</div>
          <DataTable
            columns={congCols}
            rows={cong}
