...
export default function PoliceReports() {
  ...
  const catalogQ = useQuery({
    queryKey: ["violation-catalog"],
    queryFn: () => getAdapter().violationCatalog(),
  });
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-xs font-medium hover:bg-muted"
            >
              <ShieldAlert className="h-3.5 w-3.5" /> Detect
            </button>
            <button
              onClick={() => void exportPolicePdf(filters)}
            ...
            >
              <FileDown className="h-3.5 w-3.5" /> {t("reports.exportPdf")}
            </button>
          </div>
        }
      />
      ...
      <div className="px-4 pt-3">
        <StatGrid className="lg:grid-cols-6">
          ...
          <StatCard
            icon={Clock}
            label="Pending Cases"
            value={incidents.length - withChallan.length}
            tone={incidents.length - withChallan.length > 0 ? "warn" : "ok"}
            loading={q.isLoading}
          />
          <StatCard
            icon={ReceiptText}
            label="Challans Issued"
            value={withChallan.length}
            tone="ok"
            loading={q.isLoading}
          />
          <StatCard
            icon={ShieldAlert}
            label="Customs Flags"
            value={customsQ.data?.alerts?.length ?? "—"}
            tone={(customsQ.data?.alerts?.length ?? 0) > 0 ? "critical" : "ok"}
...
function FilterBar(props: {
  ...
  const opt = (arr: string[]) => [
    { value: "", label: "All" },
    ...arr.map((o) => ({ value: o, label: o })),
  ];
  const dateCls =
    "h-9 rounded-md border border-border bg-background px-2 text-[13px] outline-none focus:ring-2 focus:ring-primary/20";
  return (
    <Card className="flex flex-wrap items-end gap-3 p-3">
      <label className="text-[11px] text-muted-foreground">
        Kind
        <div className="mt-1">
          <FilterSelect value={props.kind} onChange={props.setKind} options={opt(KINDS)} />
        </div>
      </label>
      <label className="text-[11px] text-muted-foreground">
        Gate
        <div className="mt-1">
          <FilterSelect value={props.gate} onChange={props.setGate} options={opt(GATES)} />
        </div>
      </label>
      <label className="text-[11px] text-muted-foreground">
        Severity
      ...
      <label className="text-[11px] text-muted-foreground">
        ...
        <input
          type="datetime-local"
          value={props.since}
          onChange={(e) => props.setSince(e.target.value)}
          className={`mt-1 block ${dateCls}`}
        />
      </label>
      <label className="text-[11px] text-muted-foreground">
        To
        <input
          type="datetime-local"
          value={props.until}
          onChange={(e) => props.setUntil(e.target.value)}
          className={`mt-1 block ${dateCls}`}
        />
      </label>
    </Card>
  ...
  const columns: Column<PoliceIncident>[] = [
    {
      ...
      render: (i) => fmtDateTimeIST(i.ts),
    },
    { key: "kind", header: "Kind", render: (i) => i.kind },
    {
      key: "sev",
      header: "Severity",
      render: (i) => <StatusChip label={i.severity} tone={sevTone(i.severity)} />,
    },
    {
      key: "plate",
      header: "Plate",
      ...
      render: (i) => (
        <span
          className={
            i.plate && i.plate === highlightPlate
              ? "animate-pulse rounded bg-severity-warning/20 px-1"
              : ""
          }
        >
          {i.plate ?? "—"}
    ...
    { key: "gate", header: "Gate", render: (i) => i.gate_id?.replace("G-", "") ?? "—" },
    {
      ...
      render: (i) => i.rc?.owner_name_masked ?? "—",
    },
    {
      key: "ev",
      header: "Evidence",
      align: "right",
      render: (i) => (i.evidence_url ? <span className="text-severity-info">photo</span> : "—"),
    },
  ];
  return (
    <DataTable
      ...
      search={(i, q) =>
        `${i.plate ?? ""} ${i.kind} ${i.gate_id ?? ""} ${i.rc?.owner_name_masked ?? ""}`
          .toLowerCase()
          .includes(q)
      }
  ...
  const columns: Column<PoliceIncident>[] = [
    {
      ...
      className: "font-mono",
      render: (i) =>
        String((i.challan as any)?.challan_no ?? (i.challan as any)?.echallan_id ?? i.id),
    },
    ...
    { key: "kind", header: "Offence", render: (i) => i.kind },
    {
      ...
      render: (i) => {
        const f =
          (i.challan as any)?.fine_inr ??
          (i.challan as any)?.amount ??
          (i.challan as any)?.total_fine;
...
function CustomsTable({
  ...
  return (
    <DataTable
      ...
      status={status}
      onRetry={onRetry}
      emptyLabel="No customs flags in RDS."
      search={(a, q) =>
        `${String(a.payload?.flag ?? "")} ${a.plate ?? ""} ${a.severity}`.toLowerCase().includes(q)
      }
...
function TrafficReports({ incidents }: { incidents: PoliceIncident[] }) {
  ...
  if (incidents.length === 0)
    return (
      <Card className="p-0">
        <EmptyState>No incident data to report.</EmptyState>
      </Card>
    );
  return (
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 4, right: 8, left: -18, bottom: 0 }}>
            ...
            <Bar dataKey="count" radius={[3, 3, 0, 0]} fill={STATUS.info}>
              {coloured &&
                data.map((_, i) => <Cell key={i} fill={KIND_COLOURS[i % KIND_COLOURS.length]} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
...
function ViolationsTab({ catalogQ, incidents }: { catalogQ: any; incidents: PoliceIncident[] }) {
  ...
  const columns: Column<any>[] = [
    ...
    { key: "section", header: "Section", render: (c) => c.section ?? "—" },
    {
      ...
      align: "right",
      className: "tabular-nums",
      render: (c) => (c.fine_inr != null ? `₹${Number(c.fine_inr).toLocaleString("en-IN")}` : "—"),
    },
    {
      key: "cases",
      header: "Cases (filtered)",
      align: "right",
      className: "tabular-nums",
      render: (c) => counts.get(c.kind) ?? 0,
    },
  ];
  return (
    <Card className="overflow-hidden">
      <DataTable
        ...
        emptyLabel="No violation catalog available."
        search={(c: any, q) => `${c.label ?? ""} ${c.kind}`.toLowerCase().includes(q)}
...
function CarbonTab({ carbonQ }: { carbonQ: any }) {
  ...
  const byClass = useMemo(
    () =>
      c
        ? Object.entries(c.by_class).map(([name, kg]) => ({ name, count: Math.round(Number(kg)) }))
        : [],
    [c],
  );
  if (carbonQ.isLoading)
    return (
      <Card className="p-0">
        <EmptyState>Loading carbon report…</EmptyState>
      </Card>
    );
  if (carbonQ.isError || !c)
    return (
      <Card className="p-0">
        <EmptyState>No carbon data available.</EmptyState>
      </Card>
    );
  return (
    <div className="space-y-3">
      <StatGrid className="lg:grid-cols-4">
        ...
        <StatCard
          icon={Leaf}
          label="Moving"
          value={`${Math.round(c.by_source.moving)} kg`}
          tone="info"
        />
        <StatCard
          icon={Clock}
          label="Idle"
          value={`${Math.round(c.by_source.idle)} kg`}
          tone="warn"
        />
      </StatGrid>
          ...
          <>
            ...
            <div className="space-y-4 p-5">
              ...
              <button
                ...
                className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-severity-info/40 bg-severity-info/10 px-4 py-2.5 text-sm font-semibold text-severity-info transition-colors hover:bg-severity-info/20"
              >
