...
import { PageContainer, PageHeader, StatGrid, StatCard, StatusChip, type Tone } from "@/components/ui/dtccc";
...
const BACKING_TONE: Record<Backing, Tone> = { LIVE: "ok", SIM: "warn", RDS: "info", EPHEMERAL: "neutral" };
const HEALTH_COLOUR: Record<Health, string> = { HEALTHY: STATUS.ok, DEGRADED: STATUS.warning, DOWN: STATUS.critical, UNKNOWN: STATUS.unknown };
...
export default function SystemHealth() {
  ...
  const fastagQ = useQuery({ queryKey: ["fastag-health"], queryFn: () => getAdapter().fastagHealth(), refetchInterval: 10000, retry: false });
  const gatewayQ = useQuery({ queryKey: ["gateway-health"], queryFn: () => api.health(), refetchInterval: 5000, retry: false });
  const [drawer, setDrawer] = useState<{ title: string; api?: string; source?: string } | null>(null);

  const sources = sourcesQ.data ?? [];
  const cameras = camerasQ.data ?? [];
  const match = (pred: (s: string) => boolean) => sources.filter((s) => pred(s.source.toLowerCase()));
  ...
  const services: Service[] = useMemo(() => {
    ...
    const gwReachable = gatewayQ.data?.status === "ok" || (!sourcesQ.isError && sources.length > 0) || !camerasQ.isError;
    const gwOk = gwReachable;
    const dbUp = !!fastagQ.data?.db || sources.length > 0;

    const external = worstOf(match((s) => /vahan|sarathi|fastag|congestion|traffic|here|tomtom/.test(s)));
    ...
    return [
      {
        key: "gateway", name: "Gateway", icon: Server,
        health: gwOk ? "HEALTHY" : "DOWN",
        backing: "EPHEMERAL", heartbeat: now, responseMs: null,
        detail: gatewayQ.data ? `${gatewayQ.data.ws_clients} WS clients` : gwOk ? "reachable · API serving" : "unreachable", api: undefined,
      },
      {
        key: "database", name: "Database (RDS)", icon: Database,
        health: dbUp ? "HEALTHY" : "DOWN", backing: "RDS", heartbeat: now, responseMs: null,
        detail: fastagQ.data?.db ? `PostgreSQL · ${fastagQ.data.db}` : "PostgreSQL",
      },
      {
        key: "kafka", name: "Kafka / Event Bus", icon: Radio,
        health: gwOk ? "HEALTHY" : gatewayQ.isError ? "DOWN" : "UNKNOWN", backing: "EPHEMERAL",
        heartbeat: gatewayQ.dataUpdatedAt ? new Date(gatewayQ.dataUpdatedAt).toISOString() : null, responseMs: null,
        detail: gatewayQ.data ? `streaming · ${gatewayQ.data.ws_clients} subscribers` : "event stream", api: "alerts",
      },
      {
        key: "cameras", name: "Cameras (ANPR)", icon: Camera,
        health: camHealthy ? (camLive ? "HEALTHY" : "DEGRADED") : "UNKNOWN", backing: backingFromPath(camPath),
        heartbeat: now, responseMs: null, detail: `${cameras.length} camera feeds`, api: "anpr",
      },
      {
        key: "gps", name: "GPS / Telemetry", icon: Satellite,
        health: healthFromState(gps?.state), backing: backingFromPath(gps?.last_decision_path),
        heartbeat: gps?.last_ok ?? null, responseMs: gps?.latency_p95_ms ?? null, detail: gps?.source ?? "trucking telemetry", api: "trucks", source: gps?.source,
      },
      {
        key: "ai", name: "AI Services", icon: Cpu,
        health: healthFromState(ai?.state), backing: backingFromPath(ai?.last_decision_path),
        heartbeat: ai?.last_ok ?? null, responseMs: ai?.latency_p95_ms ?? null, detail: "anomaly · forecaster · OCR", api: "alerts", source: ai?.source,
      },
      {
        key: "storage", name: "Storage (MinIO)", icon: HardDrive,
        health: dbUp ? "HEALTHY" : "UNKNOWN", backing: "RDS", heartbeat: now, responseMs: null,
        detail: "evidence object store",
      },
      {
        key: "external", name: "External APIs", icon: Globe,
        health: healthFromState(external?.state), backing: backingFromPath(external?.last_decision_path),
        heartbeat: external?.last_ok ?? null, responseMs: external?.latency_p95_ms ?? null, detail: "Vahan · Sarathi · FASTag · Traffic", api: "vahan", source: external?.source,
      },
    ];
  }, [sources, cameras, fastagQ.data, gatewayQ.data, gatewayQ.dataUpdatedAt, sourcesQ.isError, camerasQ.isError]);
  ...
  const updatedAt = Math.max(sourcesQ.dataUpdatedAt || 0, camerasQ.dataUpdatedAt || 0, gatewayQ.dataUpdatedAt || 0);
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        onRefresh={() => { void sourcesQ.refetch(); void camerasQ.refetch(); void gatewayQ.refetch(); }}
      ...
      <div className="px-4 pt-3">
        <StatGrid className="lg:grid-cols-4">
          ...
          <StatCard icon={Activity} label="Degraded" value={degraded} tone={degraded > 0 ? "warn" : "ok"} />
      ...
      {/* Service cards */}
      {sourcesQ.isLoading ? (
        <div className="flex items-center gap-2 p-6 text-sm text-muted-foreground"><Spinner /> {t("health.loadingSourceHealth")}</div>
      ) : (
        <div className="grid grid-cols-1 gap-3 px-4 py-3 sm:grid-cols-2 xl:grid-cols-4">
          {services.map((s) => (
            <ServiceCard key={s.key} svc={s} onClick={s.api ? () => setDrawer({ title: s.name, api: s.api, source: s.source }) : undefined} />
          ))}
        </div>
      )}
      ...
      <div className="px-4 pb-3">
        <Card className="p-3">
          ...
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4 xl:grid-cols-6">
            ...
            {cameras.length === 0 && <span className="text-xs text-muted-foreground">No camera feeds reported.</span>}
          </div>
        </Card>
      </div>
...
function ServiceCard({ svc, onClick }: { svc: Service; onClick?: () => void }) {
  ...
  const Icon = svc.icon;
  const body = (
    <Card className={`h-full space-y-2.5 p-3 ${onClick ? "transition-colors hover:border-primary/60" : ""}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg" style={{ backgroundColor: `${colour}1a`, color: colour }}>
      ...
      <div className="flex items-center gap-1.5">
        <StatusChip label={svc.health} tone={svc.health === "HEALTHY" ? "ok" : svc.health === "DEGRADED" ? "warn" : svc.health === "DOWN" ? "critical" : "neutral"} />
      ...
      <dl className="grid grid-cols-2 gap-x-2 gap-y-1 text-[11px] text-muted-foreground">
        ...
        <dd className="text-right text-foreground">{svc.heartbeat ? relativeAge(svc.heartbeat) : "—"}</dd>
        <dt>Response p95</dt>
        <dd className="text-right tabular-nums text-foreground">{svc.responseMs != null ? `${Math.round(svc.responseMs)} ms` : "—"}</dd>
        <dt>Status</dt>
        <dd className="text-right text-foreground">{svc.health === "HEALTHY" ? "Operational" : svc.health}</dd>
      </dl>
      <div className="truncate border-t border-border/60 pt-2 text-[11px] text-muted-foreground" title={svc.detail}>{svc.detail}</div>
    </Card>
  );
...
function CameraChip({ cam, onClick }: { cam: CameraHealth; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex items-center justify-between rounded-md border border-border bg-background px-2.5 py-2 text-left hover:border-primary/60">
...
function LogDrawer({ drawer, onClose }: { drawer: { title: string; api?: string; source?: string } | null; onClose: () => void }) {
  const { t } = useTranslation();
  const q = useQuery({
  ...
  const rows: Decision[] = (q.data ?? []).filter((d) => !drawer?.source || !d.key || d.key === drawer.source || d.api === drawer.api);
  return (
      ...
      <DialogContent side="right">
        {drawer && (
          <>
            ...
            <div className="p-4">
              {q.isLoading ? (
                <div className="flex items-center gap-2 text-sm text-muted-foreground"><Spinner /> {t("health.loadingDecisions")}</div>
              ) : rows.length === 0 ? (
                  ...
                  {rows.slice(0, 100).map((d, i) => (
                    <li key={i} className="rounded-md border border-border/60 bg-background px-3 py-2 text-xs">
                      ...
                      <div className="mt-1 flex justify-between text-muted-foreground">
                        ...
                        <span className="tabular-nums">{d.latency_ms != null ? `${Math.round(d.latency_ms)} ms` : ""}</span>
                      </div>
                    </li>
                  ))}
                </ul>
