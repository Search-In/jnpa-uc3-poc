// CFS-ECY — Data Upload (module 13 sub-module). Admin/CONTROL_ROOM/CUSTOMS.
// Mirrors the Shipping Lines Data Upload UX exactly: pick facility (CFS/ECY) →
// Download template → pick CSV/XLS/XLSX → Validate (preview + errors, no import) →
// Import (idempotent) → history refresh. Everything flows through /api/cfs-ecy/* —
// nothing mocked; re-uploading the same file is a no-op (sha256 dedup), rows that
// already exist are skipped by the (facility, container, event, mode) unique key,
...
const FACILITIES = [
  ...
  { value: "ECY", label: "ECY — Empty Container Yard" },
];
const BTN =
...
export default function CfsEcyUploadPanel() {
  const qc = useQueryClient();
  const [facility, setFacility] = useState("CFS");
  ...
  const historyQ = useQuery({
    queryKey: ["cfs-ecy-uploads"],
    queryFn: () => api.cfsEcyUploads({ limit: 25 }),
  });
  ...
  const validateMut = useMutation({
    mutationFn: () => api.cfsEcyUploadValidate(facility, file as File),
    onSuccess: (res) => {
      setValidation(res);
      setImportResult(null);
    },
  ...
  const importMut = useMutation({
    mutationFn: () => api.cfsEcyUpload(facility, file as File),
    onSuccess: (res) => {
      setImportResult(res);
      // refresh history AND the browse-tab movements/stats (all cfs-ecy read queries)
      qc.invalidateQueries({
        predicate: (q) => Array.isArray(q.queryKey) && String(q.queryKey[0]).startsWith("cfs-ecy"),
      });
    },
  });
  ...
  const histCols: Column<any>[] = [
    ...
    { key: "facility_type", header: "Facility", render: (r) => r.facility_type ?? "—" },
    {
      key: "source_file",
      header: "File",
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Step 1 — pick facility + template + file */}
      <Card className="p-4">
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex flex-col gap-1">
            <label className="text-[12px] font-medium text-muted-foreground">Facility</label>
            <FilterSelect
              label="Facility"
              value={facility}
              onChange={(v) => {
          ...
          <button
            type="button"
            className={BTN}
            onClick={() => api.cfsEcyDownloadTemplate(facility)}
          >
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm font-semibold">
            ...
            <span className="text-foreground">
              {importResult.status === "SKIPPED_DUPLICATE"
                ...
                : `${importResult.imported ?? 0} imported · ${importResult.skipped ?? 0} already present · ${importResult.invalid ?? 0} invalid skipped`}
