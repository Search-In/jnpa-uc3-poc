// Shipping Lines — Data Upload (module 4 sub-module). Admin/CONTROL_ROOM/CUSTOMS.
// Mirrors the Performance Data Upload UX exactly: Download template → pick IAL/EAL/EDO
// + CSV/XLS/XLSX → Validate (preview + errors, no import) → Import (idempotent) →
// history refresh. Everything flows through /api/shipping-lines/* — nothing mocked;
// re-uploading the same file is a no-op (sha256 dedup) and invalid rows are skipped.
...
const LIST_TYPES = [
  { value: "IAL", label: "IAL — Import Advance List" },
  { value: "EAL", label: "EAL — Export Advance List" },
  { value: "EDO", label: "EDO — Electronic Delivery Order" },
];
const BTN =
...
export default function ShippingUploadPanel() {
  const qc = useQueryClient();
  const [listType, setListType] = useState("IAL");
  ...
  const historyQ = useQuery({
    queryKey: ["sl-uploads"],
    queryFn: () => api.shippingLinesUploads({ limit: 25 }),
  });
  ...
  const validateMut = useMutation({
    mutationFn: () => api.shippingLinesUploadValidate(listType, file as File),
    onSuccess: (res) => {
      setValidation(res);
      setImportResult(null);
    },
  ...
  const importMut = useMutation({
    mutationFn: () => api.shippingLinesUpload(listType, file as File),
    onSuccess: (res) => {
      setImportResult(res);
      // refresh history AND the browse-tab list + summary (all sl-* read queries)
      qc.invalidateQueries({
        predicate: (q) => Array.isArray(q.queryKey) && String(q.queryKey[0]).startsWith("sl-"),
      });
    },
  });
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex flex-col gap-1">
            <label className="text-[12px] font-medium text-muted-foreground">List type</label>
            <FilterSelect
              label="List type"
              value={listType}
              onChange={(v) => {
          ...
          <button
            type="button"
            className={BTN}
            onClick={() => api.shippingLinesDownloadTemplate(listType)}
          >
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm font-semibold">
            ...
            <span className="text-foreground">
              {importResult.status === "SKIPPED_DUPLICATE"
                ...
                : `${importResult.imported ?? 0} imported · ${importResult.invalid ?? 0} invalid skipped`}
