      ...
      <CalcitePanel heading="Simulation controls" style={{ overflow: "auto" }}>
        <div className="sim-canvas">
          <CalciteNotice open icon="lightning" kind="brand" scale="s">
            <div slot="message">
              Changes here stream live to the dashboard — open Live Operations in another tab (“Open
              dashboard”). Press play to let metrics auto-advance, or set values manually. Whatever
              you drive is highlighted and labelled on the map.
            </div>
          </CalciteNotice>
          ...
          <CalciteBlock
            open
            iconStart="clock"
            heading="Clock & playback"
            description="Master tick engine"
          >
          ...
          <CalciteBlock
            open
            iconStart="play"
            heading="Guided scenarios"
            description="Timed What-If playback"
          >
                ...
                return (
                  <div key={sc.id} className={`sim-scenario-card ${active ? "is-active" : ""}`}>
                    ...
                    <CalciteButton
                      ...
                      onClick={() =>
                        active ? simStore.stopScenario() : simStore.startScenario(sc.id)
                      }
                    >
          ...
          <CalciteBlock
            ...
            iconStart="car"
            heading="Gate queues"
            description="Trucks queued per gate"
          >
            {gates.length === 0 ? (
                ...
                {gates.map((g) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <CalciteChip
                        value={String(val)}
                        style={{ ["--calcite-chip-text-color" as never]: queueColour(val) }}
                      >
                        {val}
                      </CalciteChip>
                    </div>
          ...
          <CalciteBlock
            open
            iconStart="road-sign"
            heading="Corridor congestion"
            description="Jam factor per segment (0–10)"
          >
            {segments.length === 0 ? (
                      ...
                      <CalciteSlider
                        ...
                        onCalciteSliderInput={(e) => {
                          ...
                          if (jam === 0) simStore.clearSegment(seg.id);
                            ...
                            simStore.setSegment(seg.id, {
                              jamFactor: jam,
                              speedKmh: Math.max(4, 40 - jam * 3.6),
                            });
                          recomputeHighlights();
                        }}
                      />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <CalciteBlock
              ...
              heading="Vehicle flow"
              description="Throughput multiplier"
            >
            ...
            <CalciteBlock
              open
              iconStart="plus"
              heading="Vehicle injection"
              description="Extra trucks on the corridor"
            >
              <div className="sim-row" style={{ gridTemplateColumns: "1fr 56px" }}>
                <CalciteSlider
                  min={0}
                  ...
                  labelHandles
                  onCalciteSliderInput={(e) =>
                    simStore.setVehicleInjection(
                      Number((e.target as unknown as { value: number }).value),
                    )
                  }
                />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <CalciteBlock
              open
              iconStart="security"
              heading="Customs scan"
              description="Pending scans in the queue"
            >
              {sim.scanQueue != null && (
                <div style={{ marginBottom: 8 }}>
                  <CalciteButton
                    ...
                    iconStart="x"
                    onClick={() => simStore.setScanQueue(null)}
                  >
                    Clear override
                  </CalciteButton>
                </div>
            ...
            <CalciteBlock
              open
              iconStart="grid"
              heading="Parking pool"
              description="Availability delta (± slots)"
            >
              <div className="sim-row" style={{ gridTemplateColumns: "1fr 56px" }}>
                <CalciteSlider
                  min={-200}
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              {INCIDENT_KINDS.map((kind) => (
                <CalciteButton
                  ...
                  iconStart="exclamation-mark-triangle"
                  onClick={() =>
                    simStore.injectIncident(
                      kind,
                      INCIDENT_SEVERITY[kind] ?? "warning",
                      gates[0]?.id,
                    )
                  }
                >
              ...
              {openIncidents > 0 && (
                <CalciteButton
                  ...
                  appearance="transparent"
                  iconStart="check"
                  onClick={() => simStore.clearIncidents()}
                >
                  Resolve all
                </CalciteButton>
              )}
          ...
          <p className="sim-meta">
            Gates: {gates.length} · segments: {segments.length} · highlighted assets:{" "}
            {sim.highlights.length}
          </p>
        </div>
      </CalcitePanel>
    </CalciteShell>
